...
import static com.doskoch.fpm.web5.controller.navigation.PageNavigation.BROKEN;
...
public class BrokenCommand implements ICommand {
    @Override
    public Router execute(HttpServletRequest request) throws CommandException {
        var session = request.getSession();
        session.setAttribute(ATTRIBUTE_CURRENT_PAGE, BROKEN);

        List<Car> cars = null;
        ICarService dao = new MySqlCarService();
        try {
            cars = dao.selectBrokenCars();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        return new Router(BROKEN, FORWARD);
    }
}
